...
interface TypingTestProps {
  onComplete: (result: TestResult, isLevelUp?: boolean, newLevel?: number) => void;
}
...
const TypingTest: React.FC<TypingTestProps> = ({ onComplete }) => {
  ...
  const finishTest = async (finalInput: string, finalTime: number) => {
    ...
    // Boost XP a bit to make leveling faster
    const xpEarned = Math.round(((calculatedWpm * 1.5) * (calculatedAcc / 100)) + (calculatedWpm / 2));
    ...
    if (user) {
      ...
      const updatedUser = await mockDb.saveResult(result);
    ...
    onComplete(result, isLevelUp, newLevel);
  };
