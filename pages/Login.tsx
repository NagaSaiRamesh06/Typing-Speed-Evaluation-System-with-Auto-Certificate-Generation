...
const Login: React.FC<LoginProps> = ({ initialIsLogin = true }) => {
  ...
  const toggleMode = () => {
      ...
      navigate(newMode ? '/login' : '/signup', { replace: true, state: location.state });
  };
      ...
      <div className="max-w-md w-full bg-white dark:bg-slate-800 rounded-2xl shadow-xl p-8 border border-slate-200 dark:border-slate-700">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-slate-500 dark:text-slate-400 mt-2 text-sm">
            {location.state?.from ? 'Please sign in to access that page.' : (isLogin ? 'Enter your details to access your dashboard.' : 'Start your typing journey today.')}
          </p>
        </div>
