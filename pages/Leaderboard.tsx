          ...
          <table className="w-full text-left">
            <thead className="bg-slate-100 dark:bg-slate-900/50 border-b border-slate-200 dark:border-slate-700">
              <tr>
                ...
                <th className="px-6 py-4 text-xs font-semibold text-slate-500 uppercase text-left pl-10">Level Title</th>
                ...
                return (
                  <tr key={entry.userId} className={`hover:bg-slate-50 dark:hover:bg-slate-700/30 transition-colors ${index < 3 ? 'bg-indigo-50/30 dark:bg-indigo-900/10' : ''}`}>
                    ...
                    <td className="px-6 py-4 pl-10">
                        <span className={`inline-flex items-center gap-2 px-2.5 py-1 rounded-full text-xs font-medium ${rankInfo.bg} ${rankInfo.color}`}>
                           <i className={`fa-solid ${rankInfo.icon}`}></i>
                           {rankInfo.title} (Lvl {entry.level})
                        </span>
                    </td>
